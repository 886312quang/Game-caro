...
function Game(props) {
  ...
  const [avatarSrc, setAvatarSrc] = useState(
    localStorage.getItem('avatar_' + userInfo.username) || defaultAvatar
  );
  ...
  history.map((step, move) => {
    const content = move
      ? `Xin về lượt #${Config.makeTwoDigits(move)}:
            (${Config.makeTwoDigits(history[move].x)},${Config.makeTwoDigits(
          history[move].y
        )})`
      : `Xin chơi lại từ đầu !`;
    ...
    // Get current move
    const isDisabled =
      oneIsDisconnect || (needToDisable && message && message.startsWith('..'));
    const currentMove = (
      // eslint-disable-next-line react/no-array-index-key
      <li key={move}>
        <Button
          onClick={() => requestUndo(move)}
          variant={variant}
          disabled={isDisabled}
  ...
  function setupSocket() {
    ...
    socket.on('undo-request', function(data) {
      doConfirm(
        `Đối thủ muốn quay về lượt #${data.stepNumber}: (${data.x},${data.y}) !`,
        () => {
  ...
  function getUserAvatar() {
    var imgUrl =
      'https://firebasestorage.googleapis.com/v0/b/webnc-1612422.appspot.com/o/' +
      userInfo.username +
      '.png';
    axios
      .get(imgUrl)
